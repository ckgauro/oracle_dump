...
**Worked example** (matches the demo seed data in `data.sql`): on first boot, `client_file_location` has `ACME → ./demo-data/acme` and `GLOBEX → ./demo-data/globex`. First scan cycle:
1. `discover()` lists `./demo-data/acme/*.dmp` → finds `sample1.dmp`, `sample2.dmp`; neither exists in `dump_file_record` yet → two `INSERT ... status=DISCOVERED` rows. Same for `globex/sample1.dmp`.
